...
use Google::Ads::AdWords::v201506::AdGroupCriterion;
use Google::Ads::AdWords::v201506::AdGroupCriterionOperation;
use Google::Ads::AdWords::v201506::BiddableAdGroupCriterion;
use Google::Ads::AdWords::v201506::BulkMutateJobPolicy;
use Google::Ads::AdWords::v201506::BulkMutateJobSelector;
...
sub add_keywords_in_bulk {
  ...
  for (my $i = 1 ; $i < KEYWORD_COUNT ; $i++) {
    # Create keyword.
    my $keyword =
      Google::Ads::AdWords::v201506::Keyword->new({matchType => "BROAD"});
    ...
    my $ad_group_criterion_operation =
      Google::Ads::AdWords::v201506::AdGroupCriterionOperation->new({
        operator => "ADD",
  ...
  my $job = $client->MutateJobService()->mutate({
      ...
      policy     => Google::Ads::AdWords::v201506::BulkMutateJobPolicy->new({
  ...
  my $job_selector = Google::Ads::AdWords::v201506::BulkMutateJobSelector->new({
      jobIds         => [$job_id],
      includeStats   => 1,
      includeHistory => 1
  });
  ...
  do {
    ...
    my $jobs = $client->MutateJobService->get({selector => $job_selector});
    $job = @{$jobs}[0];
    ...
    $retries < MAX_RETRIES
    and ($job->get_status() eq "PENDING"
      or $job->get_status() eq "PROCESSING"));
  ...
  my $job_result =
    $client->MutateJobService->getResult({selector => $job_selector})
