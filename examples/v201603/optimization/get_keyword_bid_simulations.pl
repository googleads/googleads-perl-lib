...
sub get_keyword_bid_simulations {
  ...
  # Create predicates.
  my $adgroup_predicate = Google::Ads::AdWords::v201603::Predicate->new({
      field    => "AdGroupId",
      operator => "IN",
      values   => [$ad_group_id]});
  my $criterion_predicate = Google::Ads::AdWords::v201603::Predicate->new({
      field    => "CriterionId",
      operator => "IN",
      values   => [$keyword_id]});
  ...
  my $page =
    $client->DataService()
    ->getCriterionBidLandscape({serviceSelector => $selector});
