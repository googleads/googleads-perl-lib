...
sub get_keywords {
  ...
  # Create selector.
  my $ad_group_id_predicate = Google::Ads::AdWords::v201609::Predicate->new({
      field    => "AdGroupId",
      operator => "IN",
      values   => [$ad_group_id]});
  my $criteria_type_predicate = Google::Ads::AdWords::v201609::Predicate->new({
      field    => "CriteriaType",
      operator => "EQUALS",
      values   => ["KEYWORD"]});
  ...
  my $selector = Google::Ads::AdWords::v201609::Selector->new({
      ...
      ordering => Google::Ads::AdWords::v201609::OrderBy->new({
          field     => "KeywordText",
          sortOrder => "ASCENDING"
        }
      ),
