...
use Google::Ads::AdWords::v201409::PlacesLocationFeedData;
use Google::Ads::AdWords::v201409::OAuthInfo;
...
# To obtain an access token for your GMB account, run the
# generate_offline_credentials example. Make sure you are logged in as
# the same user as gmb_email_address above when you follow the link
# provided by the example, then capture the Credential's access token.
...
sub add_gmb_location_extensions {
  ...
  my $gmb_feed = Google::Ads::AdWords::v201409::Feed->new({
  ...
  my $oauth_info = Google::Ads::AdWords::v201409::OAuthInfo->new({
    httpMethod => "GET",
    httpRequestUrl => Google::Ads::AdWords::Constants::DEFAULT_OAUTH_SCOPE,
    httpAuthorizationHeader => "Bearer ${gmb_access_token}"
  });
  my $feed_data = Google::Ads::AdWords::v201409::PlacesLocationFeedData->new({
  ...
  my $feed_operation = Google::Ads::AdWords::v201409::FeedOperation->new({
    operand => $gmb_feed,
    operator => "ADD"
  });
  ...
  my $customer_matching_function =
    Google::Ads::AdWords::v201409::Function->new({
      lhsOperand => [ Google::Ads::AdWords::v201409::ConstantOperand->new({
          type => "BOOLEAN",
          booleanValue => 1
        })
      ],
