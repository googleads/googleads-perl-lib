...
sub create_and_attach_shared_keyword_set {
  ...
  my $shared_set = Google::Ads::AdWords::v201605::SharedSet->new({
  ...
  my $shared_set_result = $client->SharedSetService()->mutate({
      operations => [
        Google::Ads::AdWords::v201605::SharedSetOperation->new({
            operator => 'ADD',
            operand  => $shared_set
          })]});
  ...
  foreach my $keyword (@keywords) {
    my $keyword_criterion = Google::Ads::AdWords::v201605::Keyword->new({
        text      => $keyword,
        matchType => 'BROAD'
    });

    my $shared_criterion = Google::Ads::AdWords::v201605::SharedCriterion->new({
  ...
  my $campaign_shared_set =
    Google::Ads::AdWords::v201605::CampaignSharedSet->new({
      campaignId  => $campaign_id,
      sharedSetId => $shared_set->get_sharedSetId()});

  my $campaign_shared_set_result = $client->CampaignSharedSetService->mutate({
      operations => [
        Google::Ads::AdWords::v201605::CampaignSharedSetOperation->new({
            operator => 'ADD',
