...
use Google::Ads::AdWords::v201605::Draft;
use Google::Ads::AdWords::v201605::DraftOperation;
...
sub add_draft {
  ...
  my $draft = Google::Ads::AdWords::v201605::Draft->new({
      baseCampaignId => $base_campaign_id,
      draftName      => sprintf("Test Draft #%s", uniqid())});

  # Create operation.
  my $draft_operation = Google::Ads::AdWords::v201605::DraftOperation->new({
      operator => "ADD",
      operand  => $draft
  });
  ...
  if ($result) {
    ...
    my $criterion = Google::Ads::AdWords::v201605::Language->new({
        id => 1003    # Spanish
    });
