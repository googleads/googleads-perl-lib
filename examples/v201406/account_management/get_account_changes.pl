...
use Google::Ads::AdWords::v201406::CustomerSyncSelector;
use Google::Ads::AdWords::v201406::DateTimeRange;
...
sub get_account_changes {
  ...
  my $campaigns = $client->CampaignService()->get({
    serviceSelector => Google::Ads::AdWords::v201406::Selector->new({
      fields => ["Id"]
    })
  });
  ...
  my $date_time_range = Google::Ads::AdWords::v201406::DateTimeRange->new({
    min => $min_date_time,
    max => $max_date_time
  });

  # Create selector.
  my $selector = Google::Ads::AdWords::v201406::CustomerSyncSelector->new({
    ...
    foreach my $campaign_changes (@{$account_changes->get_changedCampaigns()}) {
      ...
      if ($campaign_changes->get_campaignChangeStatus() ne "NEW") {
        ...
        printf "\tRemoved ad extensions: %s\n",
               get_formatted_list($campaign_changes->get_removedAdExtensions());
        printf "\tRemoved campaign criteria: %s\n",
               get_formatted_list($campaign_changes->
                   get_removedCampaignCriteria());
          ...
          foreach my $ad_group_changes
            ...
            if ($ad_group_changes->get_adGroupChangeStatus() ne "NEW") {
              ...
              printf "\t\tCriteria removed: %s\n",
                     get_formatted_list(
                        $ad_group_changes->get_removedCriteria());
            }
          }
        }
      }
