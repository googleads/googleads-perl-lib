...
sub get_product_category_taxonomy {
  ...
  my $results = $client->ConstantDataService()->getProductBiddingCategoryData({
    selector => $selector
  });
  ...
  if ($results) {
    ...
    foreach my $product_bidding_category_data (@{$results}) {
      my $id = $product_bidding_category_data->get_dimensionValue()->
        get_value();
      my $parent_id;
      my $name = $product_bidding_category_data->get_displayValue()->[0]->
        get_value();
      if ($product_bidding_category_data->get_parentDimensionValue()) {
        $parent_id = $product_bidding_category_data->
          get_parentDimensionValue()->get_value();
      }
