...
sub get_keywords {
  ...
  # Create selector.
  my $ad_group_id_predicate = Google::Ads::AdWords::v201406::Predicate->new({
    field => "AdGroupId",
    operator => "IN",
    values => [$ad_group_id]
  });
  my $criteria_type_predicate = Google::Ads::AdWords::v201406::Predicate->new({
    field => "CriteriaType",
    operator => "EQUALS",
    values => ["KEYWORD"]
  });
    ...
    if ($page->get_entries()) {
      foreach my $ad_group_criterion (@{$page->get_entries()}) {
        ...
        if ($ad_group_criterion->isa(
            "Google::Ads::AdWords::v201406::NegativeAdGroupCriterion")) {
