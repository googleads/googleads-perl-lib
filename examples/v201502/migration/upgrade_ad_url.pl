...
use Google::Ads::AdWords::v201502::AdUrlUpgrade;
...
sub upgrade_ad_url {
  ...
  if ($ad_group_ad) {
    ...
    my $ad_url_upgrade = Google::Ads::AdWords::v201502::AdUrlUpgrade->new({
      adId => $ad_group_ad->get_ad()->get_id(),
      finalUrl => $ad_group_ad->get_ad()->get_url()
    });

    # Upgrade the ad.
    my $upgraded_ads = $client->AdGroupAdService()->upgradeUrl({
      operations => [$ad_url_upgrade]
    });
...
sub get_ad_group_ad {
  ...
  my $ad_group_id_predicate = Google::Ads::AdWords::v201502::Predicate->new({
    field => "AdGroupId",
    operator => "EQUALS",
    values => [$ad_group_id]
  });

  my $ad_id_predicate = Google::Ads::AdWords::v201502::Predicate->new({
    field => "Id",
    operator => "EQUALS",
    values => [$ad_id]
  });
